...
async def get_anchor_cities(region_id: int, time: int=50):
    ...
    try:
        ...
        towns = towns_layers.get_towns(region_id)
        ...
        return json.loads(settlement_boundaries.to_json())
...
async def get_spatial_inequality(
    ...
    try:
        ...
        spatial_inequality = calculator.calculate_spatial_inequality()
        if level is not None:
            aggregate_territories = towns_layers.towns_api_service.get_territories_for_region(
                region_id,
                get_all_levels=True,
                level=level
            )
            ...
            return json.loads(polygon_spatial_inequality.to_json())
        logger.info(f"Spatial inequality processed successfully for region {region_id}")
        return json.loads(spatial_inequality.to_json())
    except Exception as e:
        logger.exception(e)
        raise http_exception(
            500,
...
async def cache_towns_for_region(region_id: int, force: bool=False):
        ...
        try:
            await towns_layers.get_towns(region_id)
        except Exception as e:
            logger.exception(e)
            raise http_exception(
                500,
